...
export function PostsCard({ post }: PostsCardProps) {
    ...
    const hasValidImage = post.imageUrl && post.imageUrl.startsWith("http");
      ...
      <Card className="overflow-hidden flex flex-col justify-between group hover:shadow-md transition-all duration-200 border-muted/70 bg-card">
        ...
        <div className="relative aspect-[4/3] w-full overflow-hidden bg-muted">
          {hasValidImage ? (
            ...
            <div className="flex flex-col items-center justify-center h-full w-full text-muted-foreground/60 bg-zinc-100 dark:bg-zinc-900 gap-1">
              <span className="text-2xl">🏢</span>
              <span className="text-[11px] font-medium tracking-wide">Preview Unavailable</span>
            </div>
          )}

          
          <div className="absolute top-3 left-3 flex flex-wrap gap-1.5 z-10">
            <Badge className={post.isAvailable ? "bg-emerald-600 hover:bg-emerald-600 text-white shadow-sm border-none" : "bg-destructive text-white shadow-sm border-none"}>
              {post.isAvailable ? "Available" : "Rented"}
            </Badge>
            
            {post.isPremium && (
              <Badge variant="default" className="bg-amber-500 hover:bg-amber-500 text-white shadow-sm border-none">
          ...
          <div className="flex items-center justify-between text-[11px] text-muted-foreground pt-3 border-t border-muted/60">
            <span className="flex items-center gap-1 max-w-[65%] truncate">
              ...
              <span className="truncate">
                By <strong className="font-medium text-foreground capitalize">{post.landlord?.name ?? "Ecosystem User"}</strong>
              </span>
            </span>

            
            <Link 
              href={`/tenant-dashboard/bookmark-properties/${post.id}`} 
              className="text-xs font-semibold text-primary hover:underline flex items-center gap-0.5 group/link shrink-0 cursor-pointer"
            >
              Details
